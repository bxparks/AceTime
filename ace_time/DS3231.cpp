...
#include "Util.h" // bcdToDec(), decToBcd()
#include "DateTime.h"
...
void DS3231::readDateTime(DateTime* dateTime) const {
  ...
  // Assume that the RTC stores UTC date time.
  dateTime->timeZone(TimeZone(0));
}

void DS3231::readTemperature(Temperature* temperature) const {
  Wire.beginTransmission(mAddress);
...
void DS3231::setDateTime(const DateTime& dateTime) const {
  Wire.beginTransmission(mAddress);
