...
# Map of {letter -> index}, used to track all LETTER strings, irrespective of
# policy. Created by artransformer.py.
IndexedLetters = Dict[str, int]

# Map of LETTER strings that are more than 1-character long, grouped per policy.
# Allows the 'letter' index to be local to the given ZonePolicy. Created by
# artransformer.py. map{policy_name -> map{letter -> index}}
...
class TransformerResult(NamedTuple):
    ...
    * all_letters_map: {letter -> index}
    ...
    all_letters_map: IndexedLetters
