...
* TZ Database zone files
    * C++ files generated by a code-generator from the TZ Database zone files
      `ace_time::zonedb` (270 zones and 182 links for `BasicZoneProcessor`)
    ...
    * `ace_time::zonedbx` (387 zones and 205 links for `ExtendedZoneProcessor`)
...
* [zonedb/zone_infos.h](src/ace_time/zonedb/zone_infos.h) contains `kZone*`
  ...
  zone transition rules, and intended to be used by `BasicZoneProcessor` class,
  and
* [zonedbx/zone_infos.h](src/ace_time/zonedbx/zone_infos.h) contains `kZone*`
  ...
  the entire database) intended to be used by the `ExtendedZoneProcessor` class.
