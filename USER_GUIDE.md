...
class TimeZone {
  public:
    ...
    static TimeZone forZoneProcessor(const ZoneProcessor* zoneProcessor);
    TimeZone();
...
The `BasicZoneProcessor` represents a time zone defined by the TZ Database. The
constructor accepts a pointer to a `basic::ZoneInfo`:

```C++
BasicZoneProcessor(const basic::ZoneInfo* zoneInfo);
```
...
The following example creates a `TimeZone` using a `BasicZoneProcessor` which
...
void someFunction() {
  ...
  auto tz = TimeZone::forZoneProcessor(&zoneProcessor);
...
The `ExtendedZoneProcessor` is very similar to `BasicZoneProcessor` except that
...
```C++
ExtendedZoneProcessor(const extended::ZoneInfo* zoneInfo);
```
...
The usage is the same as `BasicZoneProcessor`:
...
```C++
static BasicZoneProcessor zspecLosAngeles(&zonedb::kZoneAmerica_Los_Angeles);
static BasicZoneProcessor zspecZurich(&zonedb::kZoneEurope_Zurich);

void someFunction() {
  ...
  auto tzLosAngeles = TimeZone::forZoneProcessor(&zspecLosAngeles);
  auto tzZurich = TimeZone::forZoneProcessor(&zspecZurich);
...
objects are meant to be *opaque* and simply passed into `BasicZoneProcessor`,
`ExtendedZoneProcessor`, `BasicZoneRegistrar` and `ExtendedZoneRegistrar`. The
...
**Caveat**: This is an experimental feature. Previously, `BasicZoneProcessor`
and `ExtendedZoneProcessor` were considered immutable objects. The ZoneRegistrar
enables dynamic lookup of `ZoneInfo`, which resulted in the `BasicZoneProcessor`
and `ExtendedZoneProcessor` becoming mutable with the `setZoneInfo()` method.
...
registry and used to configure the `BasicZoneProcessor` like this:
...
static const basic::ZoneInfo* const defaultZoneInfo =
    zonedb::kZoneAmerica_Los_Angeles;
static BasicZoneProcessor zoneProcessor(defaultZoneInfo);
...
`zonedbx::kZoneRegistry` and used to configure the `ExtendedZoneProcessor` like
this:
...
static ExtendedZoneProcessor zoneProcessor(defaultZoneInfo);
